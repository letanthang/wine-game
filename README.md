...
| Game | Route | Notes |
| --- | --- | --- |
| [Counter-Strike 1.6](games/counter-strike/README.md) | **Native Xash3D FWGS** (no Wine) | `make counter-strike`, or `make counter-strike-offline MAP=de_dust2 BOTS=8` for a match against bots. `make counter-strike-dedicated` hosts a headless LAN server for other Xash3D clients (no bots, no AMXX). GoldSrc crashes under every Wine variant on macOS — see [docs/03-native-engines.md](docs/03-native-engines.md) |
