...
| Game | Route | Notes |
| --- | --- | --- |
| [Counter-Strike 1.6](games/counter-strike/README.md) | **Native Xash3D FWGS** (no Wine) | `make counter-strike`, or `make counter-strike-offline MAP=de_dust2 BOTS=8` for a match against bots. `make counter-strike-standalone` makes `~/Games/cs16` self-contained — a `play.sh` launcher plus a double-clickable `Counter-Strike 1.6.app` that need no repo and survive the folder being moved or copied to another Mac. `make counter-strike-dedicated` hosts a headless LAN server for other Xash3D clients (no bots, no AMXX). GoldSrc crashes under every Wine variant on macOS — see [docs/03-native-engines.md](docs/03-native-engines.md) |
| [CS 1.6 for Windows](games/counter-strike/windows/README.md) | **Xash3D FWGS win32-i386** (no Wine) | `make counter-strike-windows ZIP=1` packages a portable folder from this Mac — engine + CS16Client Windows binaries plus the local assets. Copy it to a Windows PC and run `play.bat`; no installer, no Steam |
