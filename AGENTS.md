...
- **Dedicated servers** are separate from the client setup and live in `games/<game-slug>/rehlds/` (currently CS 1.6 only). That stack is **Debian x86 + bash**, not macOS/zsh: ReHLDS + ReGameDLL_CS + Metamod-R + ReUnion + AMX Mod X + zBot, installed by `install.sh`, testable locally through a `linux/amd64` Docker image with i386 multi-arch (`make counter-strike-server`). No Wine involved.
